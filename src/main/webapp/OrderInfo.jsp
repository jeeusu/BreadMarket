...
if(cookies!=null) {
	for(int i=0;i<cookies.length;i++) {
		if(cookies[i].getName().equals("address"))
			address=URLDecoder.decode((cookies[i].getValue()),"utf-8");
		else if(cookies[i].getName().equals("deliveryinfo"))
			deliveryinfo=URLDecoder.decode((cookies[i].getValue()),"utf-8");
	}
}
...
<body>
	<div class="container py-4">
		    		...
		    		<table class="table table-hover">
		    			...
		    			<%
							...
							while(breadrs.next()) {
								...
								all+=price*amount;
								content+=(breadrs.getString("bname")+","+amount+",");
		    			%>
		    			<tr>
		    			...
		    			<tr>
		    				...
		    				<th class="text-center"><%=(all+delivery) %></th>
		    			<tr>
		    		</table>
		    	</div>
		  	</div>
		</div>
		<div class="text-center"><a href="OrderDeletecart.jsp?id=<%=id %>" class="btn btn-danger">홈으로 돌아가기</a></div>
		...
		<%
			...
			String historysql =""+"INSERT INTO orderhistory values (?, ?, ?, ?, ?)";
			PreparedStatement historyps=conn.prepareStatement(historysql);
			historyps.setString(1,id);
			historyps.setString(2,content);
			historyps.setInt(3,(all+delivery));
			historyps.setString(4,address);
			historyps.setString(5,infonum);
			
			int rows = historyps.executeUpdate();
			if(rows>0)
				System.out.print(id+",주문내역:"+content+" 끝,"+(all+delivery)+","+address+","+infonum);
